...
if (empty($_GET['idtransaccionesclientes'])) {
   ...
   // INICIO REPORTE
    ...
    $pdf->SetFont('Arial', '', 8);
    $pdf->Ln(5);
    $pdf->MultiCell(130, 4, utf8_decode("Estimado(a) cliente. Agradecemos su preferencia, la transacción fue efectuada con éxito. Cualquier consulta, duda y/o reclamo puede efectuarlo en nuestros teléfonos 2255-0090, 2255-0091 y 2255-0192 o a nuestro correo electrónico servicioalcliente@acomprendedores.com específicando el número de transacción que se encuentra en la parte superior de este documento. Trabajamos duro cada día para darle un excelente servicio a cada uno de nuestros clientes."));
    $pdf->Ln(-15);
    ...
    $pdf->SetTextColor(0, 0, 0);
    $pdf->SetFont('Arial', '', 7);
    $pdf->MultiCell(190, 4, utf8_decode("COMPROBANTE OFICIAL DE CANCELACIÓN Aco Emprendedores ~ " . $Gestiones->getReferenciaTransaccionDepositosRetirosCuentas()), 0, 'C');
