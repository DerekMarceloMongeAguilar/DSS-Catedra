...
?>
        ...
        <title>Aco Emprendedores | Estado Cuenta Cr&eacute;ditos Clientes </title>
        <!-- Favicon icon -->
...
<?php
        ...
        $plazo = $Gestiones->getTiempoPlazoCreditos();
        $monto = $Gestiones->getMontoFinanciamientoCreditos();

        if ($plazo > 0) {
            if ($Gestiones->getNombreProductos() == "Préstamos Hipotecarios") {
                $CalculoCuotaMensualCapital = $monto / ($plazo * 12);
            } else {
                $CalculoCuotaMensualCapital = $monto / $plazo;
            }
        } else {
            // Puedes decidir qué hacer en caso de plazo cero: asignar 0 o mostrar un mensaje
            $CalculoCuotaMensualCapital = 0;
            // También puedes registrar el error para debugging
            error_log("Error: Plazo de crédito es cero para el producto " . $Gestiones->getNombreProductos());
        }
	...
	<td width="65%"><br><br>
        ...
        echo '"><';
        echo $UrlGlobal;
        echo '<b><i style="font-size: .8rem" class="fa fa-university"></i> ESTADO DE CUENTA (';
