...
// IMPORTANDO MODELO RECUPERACION DE CUENTAS Aco Emprendedores
...
switch ($peticion_url) {
        ...
        if (empty($cifrado)) {
            ...
            if ($IniciarSesionUsuarios) {
                ...
                */
                ...
                /*
                    -> ROLES DE USUARIOS VALIDADOS [ACO EMPRENDEDORES H.A] -> ID -> VALOR ENTERO
    ...
    case "recuperar-cuentas":
        ...
        $Asunto = "Reestablecer contraseña - Dpto Sistemas Aco Emprendedores"; // ASUNTO POR DEFECTO DE CORREO
    ...
    case "cambiar-contrasenia-recuperacion":
        ...
        if (empty($_POST['val-password'])) {
            ...
            $CodigoRecuperacion = rand(10000, 99999); // CODIGO AUTOMATICO DE RECUPERACION [5 DIGITOS]
            $Nombre = "Aco Emprendedores - Soporte Sistemas"; // NOMBRE POR DEFECTO EMPRESA
            $Remitente = "sistemas@acoemprendedores.com"; // CORREO DE RECUPERACION DE CUENTAS -> EMPRESA
            $Asunto = "Cambio Contraseña - Dpto Sistemas Aco Emprendedores"; // ASUNTO POR DEFECTO DE CORREO
            $Bytes = random_bytes(5); // GENERAR CODIGO DE 5 DIGITOS - LETRAS
            $Token = bin2hex($Bytes); // ENVIAR TOKEN A INSERCCION
            // INICIALIZANDO CLASE DE ENVIO DE CORRREOS -> USUARIOS QUE RECUPERAN SU CUENTA
            $mail = new PHPMailer(true);
